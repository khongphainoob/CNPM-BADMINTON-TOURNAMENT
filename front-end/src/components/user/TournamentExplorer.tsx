    ...
    <div style={{ padding: '40px 48px', maxWidth: 1200, margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
        <div>
          ...
          <div style={{ fontSize: 14, color: 'var(--ink-2)', marginTop: 8 }}>
            Khám phá và đăng ký tham gia các giải đấu cầu lông
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ padding: 64, textAlign: 'center', background: 'var(--paper)', border: '1px dashed var(--line)', borderRadius: 12 }}>
          ...
          <div style={{ marginTop: 16, fontSize: 16, fontWeight: 600, color: 'var(--ink-2)' }}>Hiện chưa có giải đấu nào</div>
        </div>
      ) : (
              ...
              <div style={{ padding: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                  ...
                  <div style={{ 
                    ...
                    background: t.status === 'live' ? 'var(--accent)' : t.status === 'finished' ? 'oklch(0.90 0.05 160)' : 'var(--paper-2)',
                    color: t.status === 'live' ? 'white' : t.status === 'finished' ? 'oklch(0.42 0.14 160)' : 'var(--ink-3)',
                  }}>
                    {t.status}
                  </div>
                </div>
                ...
                <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--line-2)' }}>
                  <button style={{ width: '100%', padding: '10px', borderRadius: 8, background: 'var(--ink)', color: 'white', border: 'none', fontWeight: 600, cursor: 'pointer' }}>
